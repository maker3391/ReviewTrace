...
- 🔴 **`scripts/agent-api-e2e.sh` 53건을 오늘(2026-08-30) 한 번도 돌리지 않았다.** 그 스크립트는
  컨테이너와 dev 서버가 떠 있어야 도는데(`bash scripts/agent-api-e2e.sh`), 오늘은 서버·DB 를
  임의로 띄우지 않는 규칙을 지켰다. 🔴 **그런데 오늘 Agent API 가 지나는 길에 실제 변경이 있었다** —
  Project 범위 쓰기 인가(`issueInScope`) · AppError stable code 체계(응답 문구 경로) ·
