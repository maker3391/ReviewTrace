...
describe("messages", () => {
  ...
  ] as const)("%s 의 모든 Domain 값에 두 언어의 이름표가 있다", (group, values) => {
   ...
   * 🔴 **이름표가 값을 흉내내지 못하게 한다.**
   *
   * Filter 는 고른 «값»을 URL Search Param 으로 내보낸다(CLAUDE.md 8). 이름표를 값 자리에
  ...
  it("⑨ 오류 화면의 낱말이 두 언어 모두 있다", () => {
    for (const key of Object.keys(ko.errorPage) as (keyof typeof ko.errorPage)[]) {
