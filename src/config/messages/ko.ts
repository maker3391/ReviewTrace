 ...
 * 옮겨 적지 않는다**(CLAUDE.md 11). 🔴 **번역된 낱말이 Select 의 `value` 나 URL Search
...
export const ko = {
  ...
  enums: {
    ...
    status: {
      ...
      RESOLVED: "해결됨",
      IGNORED: "무시함",
      /** 🔴 「해결됨」과 다른 말이다 — Agent 가 잘못 짚은 것이다(CLAUDE.md 2). */
      FALSE_POSITIVE: "오탐",
      REOPENED: "재발생",
    } satisfies Record<IssueStatus, string>,
    /** 넓은 기술 영역. Tag·Pattern 과 다르다(CLAUDE.md 3). */
    category: {
      ARCHITECTURE: "아키텍처",
      SECURITY: "보안",
      PERFORMANCE: "성능",
  ...
  login: {
     ...
     * Review 에서 다시 쓰이는 Knowledge 다(CLAUDE.md 2·14).
     */
    showcaseDecision:
  ...
  /**
   * 🔴 **Section 이름을 풀어쓴 설명을 두지 않는다**(CLAUDE.md 16).
   *
  ...
  apiKeys: {
    ...
    /** 🔴 남긴다 — 다시 볼 수 없다는 사실을 이 화면에서 놓치면 키를 잃는다(CLAUDE.md 12). */
    issuedWarning:
  ...
  errorPage: {
    ...
    /** 🔴 `digest` 는 서버 Log 와 짝이다. Stack Trace 를 대신 보여 주지 않는다(CLAUDE.md 19). */
   ...
   * 문구다(`lib/errors.ts`) — 기계가 읽는 계약이 쿠키에 따라 흔들리면 안 된다(CLAUDE.md 13).
   */
  errors: {
    /** 알 수 없는 오류의 마지막 한 줄. 원인은 서버 Log 에만 남는다(CLAUDE.md 19). */
