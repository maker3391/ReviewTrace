 ...
 * 볼 수 없다. 그래서 발급 결과를 Dialog 로 **한 번** 보여 주고, 닫으면 State
 * 에서도 지운다.
 *
 * 🔴 **목록은 `plainToken` 도 `keyHash` 도 받지 않는다.** 서버가 아예 넘기지 않는다 —
 * 넘기면 RSC payload 로 페이지 소스에 실려 나간다.
 *
...
export function ApiKeyPanel({
 ...
 // 목록은 서버가 다시 그린다 — 여기서 State 를 손보지 않는다.
 ...
 */}
 ...
 그 키를 쓰던 Agent 는 다음 요청부터 `401` 을 받는다.
