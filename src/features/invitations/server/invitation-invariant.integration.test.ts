 ...
 * 뿐**이고, 정작 적용될 Migration 이 잘못돼 있어도 초록이다(CLAUDE.md 0 —
 * 「`db:generate` 가 만든 SQL 을 그대로 믿지 마라」).
 *
...
function liveEmailIndexMigration(): string {
  ...
  const creates = readdirSync(directory)
    ...
    .sort()
    .flatMap((name) =>
      readFileSync(join(directory, name), "utf8").split("--> statement-breakpoint"),
    )
    .map((statement) => statement.trim().replace(/;$/, ""))
    .filter(
      (statement) =>
        statement.includes(INDEX_NAME) && /create\s+unique\s+index/i.test(statement),
    );
...
describe.skipIf(!enabled)("살아 있는 초대는 (Workspace, Email) 당 하나다", () => {
  ...
  it("🔴 취소된 초대는 재초대를 막지 않는다 (옛 predicate 로 되돌리면 막힌다)", async () => {
    await withLiveEmailIndex(async (tx) => {
      ...
      await revokeInvitation(
        {
          ...
          invitationId: (await liveRows(tx, workspace, GUEST))[0]?.id as string,
        },
        tx,
      );
      ...
      expect(
        rows.find((row) => row.revokedAt === null)?.tokenHash,
      ).toBe(hashInvitationToken(second.token));
      expect(
        rows.find((row) => row.revokedAt !== null)?.tokenHash,
      ).toBe(hashInvitationToken(first.token));
  ...
  it("🔴 «만료 + 취소»된 행은 회전 대상이 아니다 — 살아 있는 만료 행만 회전한다", async () => {
    await withLiveEmailIndex(async (tx) => {
      ...
      const revokedId = (await liveRows(tx, workspace, GUEST))[0]?.id as string;
      await revokeInvitation(
