...
describe("createInvitation", () => {
  ...
  it("🔴 회전 UPDATE 에도 「이미 멤버가 아닐 것」이 실린다", async () => {
    ...
    const executor: DbExecutor = {
      select: () => ({
        from: () => ({
          where: () => ({ for: () => Promise.resolve([{ slug: "acme" }]) }),
        }),
      }),
  ...
  it("🔴 「이미 멤버」 비교에도 저장에도 «정규화된» 이메일이 실린다", async () => {
    ...
    expect(
      sent.params.filter((param) => param === "guest@example.test"),
    ).toHaveLength(2);
...
describe("findInvitationPreview", () => {
  ...
  it("이미 수락된 초대는 보이지 않는다", async () => {
    ...
    expect(
      await findInvitationPreview("A".repeat(43), fake.executor),
    ).toBeNull();
  });
