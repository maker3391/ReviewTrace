...
export async function formatEvidenceSnapshot(
 ...
 const normalizedSource = snapshot.replace(/\r\n|\r/gu, "\n").replace(/\n$/u, "");
 return {
 code,
...
export async function EvidenceList({
 ...
 displayFormatted={displaySnapshots[group.index]?.formatted ?? false}
 ...
 displayLineStructureChanged={beforeDisplay?.lineStructureChanged ?? false}
 ...
 displayLineStructureChanged={afterDisplay?.lineStructureChanged ?? false}
...
function CodeViewport({
 ...
 const highlighted =
 language === null ? null : highlighter.highlight(language.highlighter, code);
 ...
 >
 <span>{line.changed ? (changeKind === "BEFORE" ? "−" : "+") : ""}</span>
 <span>
