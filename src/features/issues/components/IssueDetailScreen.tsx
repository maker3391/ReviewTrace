 ...
 */
 ...
 곁 정보 단이 `20rem`(320px)로 «고정»이라, 갈리는 순간 본문이 받는 폭은
 `창 − 사이드바(256) − 여백 − 320 − gap` 이다. `lg` 에서 그것을 계산하면 본문이
 **365px** 인데, 한 단으로 서는 900px 창에서는 **581px** 다 —
 **창을 넓혔는데 본문이 216px 좁아진다.** 실측이다.

 ```
 창    1920  1440  1280  1152  1024 | 900  768
 본문  1036   765   605   493   365 | 581  449
                                 ^^^ 여기만 아래보다 좁다
 ```

 🔴 **곁 단을 유동으로 바꾸지 않는다.** 그 단에 든 것(상태 전이·위치·식별)은 폭이
 줄면 값이 접혀 두 줄이 되는 종류라, 좁혀서 얻는 것보다 잃는 것이 크다.
 갈리는 «시점»을 늦추는 쪽이 맞다 — `xl` 에서 본문은 605px 로, 한 단(1152=833)보다는
 좁아도 읽을 수 있는 폭이다.
      */}
