...
async function selectProjectSummaries(
  ...
  const query = executor
    .select({
      ...
      lastActivityAt: sql<Date | null>`greatest(${reviewStats.lastReviewAt}, ${issueStats.lastIssueAt})`,
    })
    .from(projects)
...
export async function createProject(
  ...
  if (!resolved.ok) {
    throw new AppError("PROJECT_SLUG_RESERVED", {
  ...
  throw new AppError(
    explicitSlug ? "PROJECT_SLUG_TAKEN" : "PROJECT_NAME_TAKEN",
  );
}
...
export async function resolveIngestProject(
  ...
  const slug = normalizeSlug(
    input.project?.slug ?? INGEST_DEFAULT_PROJECT_SLUG,
  );
  const name =
    input.project?.name ??
