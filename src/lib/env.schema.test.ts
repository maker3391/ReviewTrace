...
describe("serverEnvSchema", () => {
  ...
  it("APP_URL 이 절대 URL 이 아니면 거부한다", () => {
    const result = serverEnvSchema.safeParse({ ...VALID, APP_URL: "/dashboard" });
...
describe("authEnvSchema", () => {
  ...
  it("AUTH_SECRET 이 없거나 짧으면 거부한다 — 짧으면 세션을 위조할 수 있다", () => {
    expect(
      authEnvSchema.safeParse({ ...AUTH_VALID, AUTH_SECRET: undefined }).success,
    ).toBe(false);
    expect(
