...
export function createClient({ apiUrl, apiKey }) {
  ...
  return {
    ...
    addActivity: (issueId, body) =>
      call("POST", `/issues/${encodeURIComponent(issueId)}/activities`, { body }),
    updateStatus: (issueId, body) =>
      call("PATCH", `/issues/${encodeURIComponent(issueId)}`, { body }),
    getIssue: (issueId) => call("GET", `/issues/${encodeURIComponent(issueId)}`),
 ...
 * 우리가 아는 뜻만 적는다(CLAUDE.md 19).
 */
function toApiError(status, parsed) {
  const code = parsed?.error?.code ?? null;
  const serverMessage = typeof parsed?.error?.message === "string" ? parsed.error.message : null;
  ...
  if (status === 400 || status === 422) {
    return new ApiError(
      serverMessage ?? "보낸 값이 계약에 맞지 않다.",
      { status, code },
    );
  }
  if (status === 429) {
    return new ApiError("요청이 너무 잦다. 잠시 뒤 다시 시도해라.", { status, code });
  }
